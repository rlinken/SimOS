...
export default function WidgetFitting() {
  const { facilityId } = useParams();
  const { toast } = useToast();
  const [selectedFitter, setSelectedFitter] = useState<string | null>(null);
  ...
  const { data: availability, isLoading: availabilityLoading, error: availabilityError } = useQuery<TimeSlot[]>({
    ...
    queryFn: async () => {
      const response = await fetch(
        `/api/widget/fitting-availability/${facilityId}/${selectedFitter}?date=${selectedDate.toISOString()}`
      );
  ...
  const createFittingBookingMutation = useMutation({
    mutationFn: async (data: any) => {
      return apiRequest("POST", `/api/widget/fitting-bookings/${facilityId}`, data);
    },
    onSuccess: () => {
      toast({
        title: "Fitting booked!",
      ...
      queryClient.invalidateQueries({
        queryKey: [`/api/widget/fitting-availability/${facilityId}/${selectedFitter}`],
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const handleBooking = () => {
    if (!selectedTime || !customerName || !customerEmail || !selectedFitter) {
      toast({
    ...
    createFittingBookingMutation.mutate({
  ...
  const primaryColor =
    widgetConfig?.primaryColor || facility?.primaryColor || "#f97316";
  const accentColor =
    widgetConfig?.accentColor || facility?.accentColor || "#fb923c";
  ...
  return (
    ...
    >
      <style>
        {`
          .widget-fitting-btn-primary {
            background-color: ${primaryColor};
            color: white;
          }
          .widget-fitting-btn-primary:hover {
            opacity: 0.9;
          }
        `}
      </style>
      ...
      <div className={`mx-auto ${containerClass}`}>
        ...
        {widgetConfig?.showLogo !== false && facility && (
          <div className="text-center mb-8">
            ...
            <h1
              ...
              data-testid="text-fitting-title"
            >
              {widgetConfig?.customTitle || "Book a Club Fitting"}
            </h1>
            {widgetConfig?.showDescription !== false && (
              <p className="text-lg opacity-80">
                {widgetConfig?.customDescription ||
                  "Get professionally fitted by our experts"}
              </p>
            )}
          </div>
        )}

        {/* Club Fitter Selection */}
        <Card className="p-6 shadow-lg mb-6">
          ...
          <Select value={selectedFitter || ""} onValueChange={setSelectedFitter}>
            <SelectTrigger className="h-12" data-testid="select-club-fitter">
              <SelectValue placeholder="Choose a club fitter..." />
            </SelectTrigger>
            <SelectContent>
              {fitters?.map((fitter) => (
                    ...
                    return (
                      <button
                        ...
                        className={`flex flex-col items-center justify-center h-20 rounded-lg border-2 transition-all ${
                          isSelected
                            ? "widget-fitting-btn-primary border-transparent shadow-md"
                    ...
                    return (
                      <button
                        ...
                        className={`h-12 rounded-lg font-medium transition-all ${
                          isSelected
                            ? "widget-fitting-btn-primary shadow-md"
                            : available
                ...
                >
                  ...
                  <div className="grid gap-2 text-sm">
                    <div className="flex justify-between">
                      <span className="opacity-70">Club Fitter:</span>
                      <span className="font-medium">
                        {selectedFitterData?.firstName} {selectedFitterData?.lastName}
                      </span>
                    </div>
                    <div className="flex justify-between">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 pt-4">
                    <button
                      ...
                      disabled={createFittingBookingMutation.isPending}
                      className="widget-fitting-btn-primary flex-1 h-12 rounded-lg font-semibold flex items-center justify-center gap-2 transition-opacity"
                      data-testid="button-confirm-fitting-booking"
                    >
                      {createFittingBookingMutation.isPending ? (
                        "Processing..."
                      ) : (
                        <>
                    ...
                    <Button
                      ...
                      data-testid="button-cancel-fitting-booking"
                    >
                      Cancel
                    </Button>
                  </div>
