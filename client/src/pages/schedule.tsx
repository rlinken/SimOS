...
export default function Schedule() {
  ...
  const getBookingPosition = (booking: Booking) => {
    ...
    // Each hour column is HOUR_COLUMN_WIDTH px, so each minute is HOUR_COLUMN_WIDTH/60 px
    const pixelsPerMinute = HOUR_COLUMN_WIDTH / 60;
  ...
  const handleSlotClick = (bay: Bay, hour: number) => {
    ...
    if (!hasBooking && bay.status === 'active') {
